"""
百度 CTR 项目 — LightGBM 基线模型

功能：
    从 target_encoded 特征中按全文件均匀抽样，训练 LightGBM 二分类模型，
    在 valid 上评估并与逻辑回归基线对比。禁止读取 holdout。
...
# 逻辑回归正式基线（用于对比）
LOGISTIC_BASELINE = {
    "roc_auc": 0.731078941430253,
    "log_loss": 0.4126315426040813,
    "valid_actual_ctr": 0.1565620590469988,
    "valid_mean_predicted_ctr": 0.2439792785058468,
}
...
def get_run_context(test_mode: bool) -> RunContext:
    ...
    if test_mode:
        return RunContext(
            test_mode=True,
            model_path=Path("models/lightgbm_baseline_test.joblib"),
            metrics_path=Path("outputs/lightgbm_baseline_test_metrics.json"),
            report_path=Path("outputs/lightgbm_baseline_test_report.txt"),
            importance_path=Path("outputs/lightgbm_baseline_test_feature_importance.csv"),
            predictions_sample_path=Path(
                "outputs/lightgbm_baseline_test_predictions_sample.csv"
            ),
            sampling_summary_path=Path(
                "outputs/lightgbm_baseline_test_sampling_summary.csv"
            ),
        )

    return RunContext(
        test_mode=False,
        model_path=Path("models/lightgbm_baseline.joblib"),
        metrics_path=Path("outputs/lightgbm_baseline_valid_metrics.json"),
        report_path=Path("outputs/lightgbm_baseline_report.txt"),
        importance_path=Path("outputs/lightgbm_baseline_feature_importance.csv"),
        predictions_sample_path=Path(
            "outputs/lightgbm_baseline_valid_predictions_sample.csv"
        ),
        sampling_summary_path=Path("outputs/lightgbm_baseline_sampling_summary.csv"),
    )
...
def train_lightgbm_model(
    ...
    """训练 LightGBM 并启用 early stopping。"""
...
def compare_with_logistic(metrics: MetricsResult) -> dict[str, str]:
    """与逻辑回归基线对比。"""

    auc_better = metrics.roc_auc > LOGISTIC_BASELINE["roc_auc"]
    logloss_better = metrics.log_loss_value < LOGISTIC_BASELINE["log_loss"]
    ctr_gap_lgbm = abs(metrics.valid_mean_predicted_ctr - metrics.valid_actual_ctr)
    ctr_gap_logistic = abs(
        LOGISTIC_BASELINE["valid_mean_predicted_ctr"]
        - LOGISTIC_BASELINE["valid_actual_ctr"]
    )
    ctr_closer = ctr_gap_lgbm < ctr_gap_logistic

    return {
        "auc_higher_than_logistic": "是" if auc_better else "否",
        "logloss_lower_than_logistic": "是" if logloss_better else "否",
        "predicted_ctr_closer_to_actual": "是" if ctr_closer else "否",
        "auc_diff": metrics.roc_auc - LOGISTIC_BASELINE["roc_auc"],
        "logloss_diff": metrics.log_loss_value - LOGISTIC_BASELINE["log_loss"],
        "predicted_ctr_gap": ctr_gap_lgbm,
        "logistic_predicted_ctr_gap": ctr_gap_logistic,
    }


def metrics_to_dict(metrics: MetricsResult, comparison: dict) -> dict:
    """指标与对比结果转 JSON。"""
    ...
    return {
        ...
        "comparison_with_logistic": comparison,
    }
...
def write_report(
    ...
    lines = [
        "百度 CTR 项目 — LightGBM 基线报告",
        "=" * 70,
        "",
        f"当前模式：{mode_label}",
        "",
        "【模型定义】",
        "  LGBMClassifier(objective='binary', n_estimators=1000, learning_rate=0.05,",
        ...
        "",
        "【与逻辑回归正式基线对比】",
        f"  逻辑回归 ROC-AUC：{LOGISTIC_BASELINE['roc_auc']:.6f}",
        f"  LightGBM ROC-AUC：{metrics.roc_auc:.6f}（差异 {comparison['auc_diff']:+.6f}）",
        f"  AUC 是否更高：{comparison['auc_higher_than_logistic']}",
        "",
        f"  逻辑回归 LogLoss：{LOGISTIC_BASELINE['log_loss']:.6f}",
        f"  LightGBM LogLoss：{metrics.log_loss_value:.6f}（差异 {comparison['logloss_diff']:+.6f}）",
        f"  LogLoss 是否更低：{comparison['logloss_lower_than_logistic']}",
        "",
        f"  逻辑回归平均预测 CTR：{LOGISTIC_BASELINE['valid_mean_predicted_ctr']:.6f}",
        f"  LightGBM 平均预测 CTR：{metrics.valid_mean_predicted_ctr:.6f}",
        f"  平均预测 CTR 是否更接近实际 CTR：{comparison['predicted_ctr_closer_to_actual']}",
        "",
        "【特征重要性 Top 15（gain）】",
    ...
    for _, row in importance_df.head(15).iterrows():
        lines.append(
            ...
            f"gain={row['gain_importance']:.4f}, split={int(row['split_importance'])}"
        )
    ...
    lines.extend(
        [
            ...
            "【说明】",
            "  - holdout 尚未使用",
            "  - 当前为抽样训练，结果需结合全量样本与 holdout 进一步确认"
...
def main() -> None:
    """主流程：分配抽样 → 构建矩阵 → 训练 LightGBM → 评估与保存。"""
    ...
    model = train_lightgbm_model(x_train_df, y_train, x_valid_df, y_valid)
    ...
    comparison = compare_with_logistic(metrics)
    ...
    context.metrics_path.write_text(
        json.dumps(metrics_to_dict(metrics, comparison), ensure_ascii=False, indent=2)
        + "\n",
        encoding="utf-8",
    )
    ...
    print(
        f"与逻辑回归 AUC 差异：{comparison['auc_diff']:+.6f}，"
        f"LogLoss 差异：{comparison['logloss_diff']:+.6f}"
    )
    print("输出文件：")
    ...
    print("LightGBM 基线测试完成，holdout 尚未使用。")
    print("=" * 70)
